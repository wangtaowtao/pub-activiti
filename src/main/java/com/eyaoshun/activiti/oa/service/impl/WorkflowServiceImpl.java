...
import com.eyaoshun.activiti.common.utils.StringUtils;
...
import com.eyaoshun.activiti.oa.service.IWorkflowService;
import com.eyaoshun.activiti.oa.utils.UserUtil;
import com.eyaoshun.activiti.oa.web.form.WorkflowBean;
...
public class WorkflowServiceImpl implements IWorkflowService {
	...
	public void saveStartProcess(WorkflowBean workflowBean, HttpSession session) {
		...
		Leavebill leavebill = iLeavebillDao.findLeavebillById(id);
		...
		variables.put("inputUser", UserUtil.getUserFromSession(session).getName());// 表示惟一用户
		/**
	...
	public Leavebill findLeavebillByTaskId(String taskId) {
		...
		Leavebill leavebill = iLeavebillDao.findLeavebillById(Long.parseLong(id));
		return leavebill;
	}
	...
	public void saveSubmitTask(WorkflowBean workflowBean, HttpSession session) {
		...
		Employee currentUser = UserUtil.getUserFromSession(session);
		...
		Task task = taskService.createTaskQuery()//
				.taskId(taskId)// 使用任务ID查询
				.taskAssignee(currentUser.getName()).singleResult();
		// 获取流程实例ID
		String processInstanceId = task.getProcessInstanceId();
		/**
		...
		if (currentUser.getManager() != null) {
			variables.put("inputUser", currentUser.getManager().getName());
		}
		...
		if (pi == null) {
			...
			bill.setState(2);
			iLeavebillDao.updateLeavebillState(bill);
		}
	}
